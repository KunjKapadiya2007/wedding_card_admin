...
const saveAsVideo = async ({ store, pixelRatio, fps, onProgress }) => {
  ...
  console.log(json,'hhhhhhhhhhhhhhhh');
...
export default DownloadButton = observer(({ store }) => {
  ...
  // Early return if store or active page is not available
  ...
  const activePage = store.activePage || store.pages[0]; // Fallback to first page
          ...
          {type !== 'json' && type !== 'html' && type !== 'svg' && (
            <>
              ...
              <div style={{ padding: '10px' }}>
                ...
                {type !== 'pdf' && (
                  <div>
                    {Math.round(activePage.computedWidth * quality)} x{' '}
                    {Math.round(activePage.computedHeight * quality)} px
                  </div>
                )}
                {type === 'gif' && (
                  <>
...
DownloadButton.propTypes = {
  store: PropTypes.shape({
    ...
    saveAsPDF: PropTypes.func,
    saveAsHTML: PropTypes.func,
    saveAsSVG: PropTypes.func,
    saveAsGIF: PropTypes.func,
